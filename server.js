...
// --- CONFIGURACIÓN DE POSTGRESQL (CÓRTEX PERSISTENTE) ---
const pool = new Pool({
...
pool.on('error', (err) => {
  console.error('[DB POOL ERROR] Error inesperado en el nodo de datos inactivo:', err.message);
});

// --- CONFIGURACIÓN DE REDIS (SINAPSIS RÁPIDA) ---
const redis = new Redis(process.env.REDIS_URL || 'redis://default:5faf81de3571e8b7146c@qhosting_redis:6379', {
  retryStrategy(times) {
    const delay = Math.min(times * 100, 3000);
    console.log(`[REDIS] Reintentando conexión (Intento ${times}). Esperando ${delay}ms...`);
    return delay;
  },
  reconnectOnError(err) {
    const targetError = 'READONLY';
    if (err.message.includes(targetError)) {
      return true; // Forzar reconexión si el nodo está en modo lectura
    }
    return false;
  },
...
redis.on('connect', () => console.log('[REDIS] Nodo de cache sincronizado correctamente.'));
redis.on('error', (err) => console.error('[REDIS ERROR] Fallo en la sinapsis de cache:', err.message));
...
const initDB = async (retries = 5) => {
  ...
  while (retries > 0) {
    try {
      ...
      console.log('[DB] Conexión establecida con el Córtex Central.');
      ...
      // 7. Tabla de Configuración de Aplicación
      await client.query(`
      ...
      if (parseInt(clientCount.rows[0].count) === 0) {
        ...
        `);
        console.log('[SEED] Cliente inicial creado en el nodo maestro.');
      }

      await client.query('COMMIT');
      console.log('[DB] Arquitectura de tablas operativa.');
      break; // Salir del bucle de reintentos
    } catch (err) {
      if (client) await client.query('ROLLBACK');
      retries--;
      console.error(`[DB ERROR] Error en migración/conexión. Reintentos restantes: ${retries}. Error: ${err.message}`);
      if (retries === 0) {
        console.error('[CRITICAL] No se pudo establecer conexión con el nodo de datos tras múltiples intentos.');
      } else {
        await new Promise(res => setTimeout(res, 5000)); // Esperar 5s antes de reintentar
      }
    } finally {
...
wss.on('connection', (ws) => {
  ws.send(JSON.stringify({ type: 'SYSTEM_HANDSHAKE', payload: { status: 'STABLE', node: 'ALPHA-NODE' } }));
});
...
// Healthcheck (Vigilancia de Orquestador)
...
server.listen(PORT, () => {
  console.log(`[NEURONADS OS] Córtex Operativo iniciado en puerto ${PORT}`);
